...
10. Open a PR from `milestone-N` into the main branch using `gh pr create`. Use a clear title and a short body summarising what the milestone delivered. Output the PR URL so the user can see it.
...
- If `gh` is not available or not authenticated, skip the PR step, note it, and continue to the next milestone.
